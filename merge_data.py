...
def read_referendum2026(region: str = 'Lombardia') -> pd.DataFrame:
    ...
    referendum['Affluenza referendum 2026'] = referendum['Votanti totais'] / referendum['Elettori totali']
...
def read_camera2022(region: str = 'Lombardia') -> pd.DataFrame:
    ...
    camera['AFFLUENZA'] = camera['VOTANTITOT'] / camera['ELETTORITOT']
    camera['AFFLUENZA_M'] = camera['VOTANTIM'] / camera['ELETTORIM']
    camera['AFFLUENZA_F'] = camera['VOTANTIF'] / camera['ELETTORIF']
...
def _prepare_distance_to_milano_for_merge(region: str = 'Lombardia') -> pd.DataFrame:
    geo = read_geodata(region=region).copy()
    geo_3035 = geo[['Codice catastale', 'Comune', 'geometry']].to_crs(epsg=3035)
    geo_3035['centroid'] = geo_3035.geometry.centroid

    milano = geo_3035.loc[geo_3035['Comune'] == 'MILANO', 'centroid']
    if milano.empty:
        raise KeyError("Nao foi possivel localizar o comune 'MILANO' na base geografica.")

    milano_centroid = milano.iloc[0]
    distance = geo_3035[['Codice catastale']].copy()
    distance['distance_to_milano_km'] = geo_3035['centroid'].distance(milano_centroid) / 1000
    distance['belfiore'] = _belfiore(distance['Codice catastale'])
    return distance[['belfiore', 'distance_to_milano_km']]
...
def merge_geodata_omi_redditi_2021(region: str = 'Lombardia') -> gpd.GeoDataFrame:
    ...
    distance_to_milano = _prepare_distance_to_milano_for_merge(region=region)
    ...
    merged = merged.merge(distance_to_milano, on='belfiore', how='left')
    merged['Anno'] = 2021
...
def export_full_dataset(
    ...
    geojson_path: str = DATA + 'full_dataset.geojson',
) -> gpd.GeoDataFrame:
    df = merge_geodata_omi_redditi_votes_2021(region=region)
    if excel_path is not None:
        df.drop(columns=['geometry']).to_excel(excel_path, index=False)
    df.to_file(geojson_path, driver='GeoJSON', index=False)
    return df


if __name__ == '__main__':
    df = export_full_dataset()
